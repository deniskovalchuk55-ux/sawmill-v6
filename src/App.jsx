...
import { fetchAllData, fetchMonthlyStats, fetchAllDebtPayments,
         ...
         saveSettings, checkAccess, writeLog, DEFAULT_CFG } from './notion.js'
...
function RatesForm({worker,onClose,onSaved,tgId,tgName}) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      else await updateStaffRates(worker.staffPageId, {rateHour:val!==''?val:undefined,ratePack:valPack!==''?valPack:undefined})
      await writeLog({tgId,name:tgName,action:`Ставки: ${worker.name}`,details:isFixed?`Фікс: ${val}`:`Год: ${val}, Пач: ${valPack}`})
      vibrate('medium')
      setSuccess(true)
      setTimeout(()=>{onSaved();onClose()},1200)
    } catch(e){setError(e.message)} finally{setLoading(false)}
